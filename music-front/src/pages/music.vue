    ...
    >
      ...
      <div class="music-music">
        ...
        <div v-if="currentMusic.id" class="music-bar-time">
          {{ currentTime | format }}/{{ totalTime % 3600 | format }}
        </div>
        <mm-progress
...
export default {
  ...
  data() {
    ...
    return {
      ...
      isMute: false, // 是否静音
      volume, // 音量大小
      pic: require('../assets/img/player_cover.png') // 封面
    }
  },
  computed: {
    picUrl() {
      return this.currentMusic.id && this.pic
        ? `url(${this.pic})`
        : `url(${defaultBG})`
    },
    percentMusic() {
  ...
  methods: {
    ...
    progressMusicEnd(percent) {
      // console.log('change process', this.totalTime * percent)
      this.audioEle.currentTime = this.totalTime * percent
    },
    // 切换播放顺序
    modeChange() {
    ...
    _getMusic(id) {
      getLMusic(id).then(res => {
        ...
        if (data.lrc === '') {
          ...
          this.nolyric = false
          this.lyric = parseLyric(data.lrc)
        }
        // 设置音乐路径
        this.audioEle.src = data.audio
        // 加载获取到总时长
        this.audioEle.load()
        this.audioEle.oncanplay = () => {
          this.totalTime = this.audioEle.duration
        }
        // 触发播放
        silencePromise(this.audioEle.play())
      })
