...
<script>
import {createFile} from "@/request/file";
...
export default {
	data() {
		return {
			// 上传组件配置项
			options: {
				target: `${this.$config.baseContext}/data/object/big`, // 上传文件-目标 URL
				chunkSize: 1024 * 1024*2, //  每个分片的大小
	...
	methods: {
		...
		handleFilesAdded(files) {
				...
				files.forEach((file) => {
          setTimeout(()=>{
            Object.assign(this.uploaderInstance.opts, {query: {...this.params}})
            file.uniqueIdentifier = "b0e6e8dc5ac3b74747482ca88ee6e819"
            file.resume()
            file.statusStr = ''
          }, 500)
        })
      ...
      this.collapse = false
		...
		handleFileSuccess(rootFile, file, response) {
			...
			if (result.code === 200) {
        let that = this
        createFile({name:file.name,object_id:result.data["object_id"],file_size:file.size.toString(),parent: this.params.parent}).then((res) => {
          console.log(res)
          if (that.filesLength === 1) {
            // 本次所有的文件均已上传完毕
            that.$message.success(`上传完毕`)
            // callType 调用此服务的方式：1 - 顶部栏，2 - 右键菜单
            if (that.callType === 1) {
              that.serviceEl.$emit('getTableDataByType')
            } else {
              that.serviceEl.getTableDataByType()
            }
            that.callback(true)
          }
        })
