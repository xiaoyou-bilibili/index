...
const fileFunction = {
	...
	getDownloadFilePath(row) {
		return `${config.baseContext}/app/file/${row.id}/data`
	},
	/**
	...
	setFileImg(file) {
		...
		} else if (fileImgMap.has(file.ext.toLowerCase())) {
			// 可以识别文件类型的文件
			return fileImgMap.get(file.ext.toLowerCase())
		} else {
			// 无法识别文件类型的文件
			return unknownImg
		}
	...
	handleFileNameClick(row, currentIndex, fileList = []) {
		...
		// 若是文件，则进行相应的预览
			...
			if (PIC.includes(row.ext.toLowerCase())) {
			...
			if (
				...
				(row.file_type === 1 && row.ext === '')
			) {
				Vue.prototype.$openBox.codePreview({ fileInfo: row, isEdit: false })
				return false
			}
			//  若当前点击项是 markdown 文档
			if (markdownFileType.includes(row.ext.toLowerCase())) {
